...
const handleSubmit = async (e, continueEditing, addAnother) => {
    ...
    try{
        ...
        console.log("Property type add successful:", response.data);
      ...
      if (addAnother) {
        ...
        clearFormFields();
        if (continueEditing) {
          navigate(`/update-property-type/${newId}`);
        }
      }
    } catch (error) {
      handleError(error);
  ...
  const handleError = (error) => {
    if (error.response) {
      console.error("Property type add failed:", error.response.data);
      ...
      console.error("Property type add failed:", "No response received");
      // Handle no response error
    } else {
      console.error("Property type add failed:", error.message);
      // Handle other errors
    }
  };
            ...
            >
              <label className="contactAdLabel">
                ...
                <input
                  ...
                  style={{height:'fit-content', padding:'10px 10px'}}
                  type="text"
                  name="name"
                  value={name}
