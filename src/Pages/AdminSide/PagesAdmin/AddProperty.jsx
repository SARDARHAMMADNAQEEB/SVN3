...
function AddProperty(property) {
  ...
  useEffect(() => {
    if (property) {
      ...
      setSaleType(property.sale_type || []);
  ...
  //     file.id === id ? { ...file, files: selectedFiles } : file
  //   );
  ...
  const handleSubmit = async (e, continueEditing, addAnother) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:8000/api/account/property/",
        requestData,
        {
          headers: {
      ...
      console.log("Property Add successful:", response.data);
      ...
      if (addAnother) {
        ...
        clearFormFields();
        if (continueEditing) {
          navigate(`/update-property/${newId}`);
        }
      }
    } catch (error) {
      handleError(error);
  ...
  //     formData.append("file", image.imageUrlFile, image.imageUrlFile.name);
  //     // formData.append(`file_${index}`, image.imageUrlFile, image.imageUrlFile.name);
  //   });
  ...
  //         id: index,
  //         image: file.full_url, // Update to match your backend response structure
  //       }));
  //     }else {
  ...
  //   }
    ...
    //   return response.data.map((file, index) => ({
    //     id: index,
    //     image: file.full_url, // Include the full_url property
    //   }));
    // } catch (error) {
    //   handleError(error);
    //   return [];
  ...
  //     // Ensure that file.file is the actual File object or contains the file data
  //     formData.append("folder", "files");
  //     formData.append("file", file.file);
  //   });
  ...
  //       return response.data.map((file, index) => ({
  //         id: index,
  //         file: file.full_url, // Include the full_url property
  //       }));
  //     } else {
  //       // Handle single file response
  //       return [
  //         {
  //           id: 0,
  //           file: response.data.full_url, // Include the full_url property
  //         },
  //       ];
  //     }
  //   } catch (error) {
  ...
  //   const updatedFiles = propertyFiles.map((file) =>
  //     file.id === id ? { ...file, file: selectedFile } : file
  //   );
  //   setPropertyFiles(updatedFiles);
  // };
  const handleFileInputChange = (e, id) => {
          ...
          <div className="contactAdcon2">
            ...
            >
              ...
              <label className="contactAdLabel">
                ...
                <input
                  className="fn-contactAd"
                  placeholder="Enter ID of property type separated by ,"
                  type="text"
                  name="property_type"
